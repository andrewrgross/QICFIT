...
for(selection in 1:53) {
  ...
  tracking.file <- rbind(tracking.file,c(selected.tissue, strftime(Sys.time(),"%a%b%d%H%M"),length(positions.loose),length(positions.neutral),length(positions.tight),length(positions.supertight)))
  
  ### Save plot
  png(filename=paste0('Z:/Data/Andrew/reference_data/gtex/sd.filtered.tables/plot.',selected.tissue, strftime(Sys.time(),"%a%b%d%H%M"),'.png'), 
      type="cairo",
      units="in", 
      width=8, 
      height=6, 
